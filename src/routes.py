...
def main():
    ...
    if request.method == "POST":
        ...
        reference_id = ref.Reference.create_reference('kirja')

        if ref.Reference.create_book_reference(reference_id, keyword, author_surname,
                                    author_name, title, year, publisher):
            return render_template("main.html",
                                    message="Viite luotu onnistuneesti!",
                                    refs=ref.Reference.get_references_normal())
        return render_template("main.html",
